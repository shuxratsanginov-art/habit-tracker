...
const QUOTES=[['Всё начинается с одного шага.','Лао-цзы'],['Дисциплина — мост между целями и результатом.','Джим Рон'],['Мы — то, что мы делаем постоянно.','Аристотель'],['Сложнее всего начать действовать.','Марк Твен'],['Маленькие ежедневные улучшения — ключ к результатам.','Роберт Кольер'],['Успех — это сумма небольших усилий, повторяемых изо дня в день.','Роберт Кольер'],['Лучшее время посадить дерево было 20 лет назад. Второе лучшее — сейчас.','Пословица'],['Не считай дни — делай так, чтобы дни считались.','Мухаммед Али'],['Фокусируйся на процессе, а не на результате.','Билл Уолш'],['Каждое утро — это шанс стать лучше.','Опра Уинфри'],['Тишина — источник великой силы.','Лао-цзы'],['Будь как вода — гибким и сильным.','Брюс Ли'],['Счастье — это не пункт назначения, а способ путешествия.','Рой Гудман'],['Делай сегодня то, что другие не хотят — завтра будешь жить как другие не могут.','Джерри Райс'],['Простота — высшая форма утончённости.','Леонардо да Винчи']];
...
function isScheduledFor(i,d){if(!i.repeat||i.repeat.type==='none')return i.date===d;const dt=new Date(d+'T12:00:00'),day=dt.getDay(),adj=day===0?7:day,dn=dt.getDate();switch(i.repeat.type){case'daily':return true;case'weekdays':case'weekly':return i.repeat.value.includes(adj);case'monthly':return i.repeat.value.includes(dn);default:return false}}
